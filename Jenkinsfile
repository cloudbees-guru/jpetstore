pipeline {
  ...
  stages {
      ...
      stage('trigger release orchestration') {
           steps {
              cloudBeesFlowCallRestApi body: '{ "parameters": { "actualParameter": [ { "actualParameterName": "artefactversion", "value": "${BUILD_ID}" }] } } ', configuration: 'flow', envVarNameForResult: '', httpMethod: 'POST', urlPath: '/pipelines?pipelineName=calculator%20pipeline&projectName=Traditional'
           }
      }
   }
}
